...
static const char *ST_HASH        = "$8$84486783037343$pYNyVrtyMalQrZLxRi7ZLQS1Fl.jkYCgASUi5P8JNb2";
...
static const int   ROUNDS_CISCO8    = 20000;
...
int module_hash_encode (MAYBE_UNUSED const hashconfig_t *hashconfig, MAYBE_UNUSED const void *digest_buf, MAYBE_UNUSED const salt_t *salt, MAYBE_UNUSED const void *esalt_buf, MAYBE_UNUSED const void *hook_salt_buf, MAYBE_UNUSED const hashinfo_t *hash_info, char *line_buf, MAYBE_UNUSED const int line_size)
{
  ...
  tmp[8] = 0; // needed for base64_encode ()

  char tmp_buf[48];

  base64_encode (int_to_itoa64, (const u8 *) tmp, 32, (u8 *) tmp_buf);

  tmp_buf[43] = 0; // cut it here

  // output

  const int line_len = snprintf (line_buf, line_size, "%s%s$%s", SIGNATURE_CISCO8, salt_buf_ptr, tmp_buf);
